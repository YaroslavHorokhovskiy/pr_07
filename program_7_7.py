...
a needless effort. Since this is the wild west, with dreadfull weather and not
much water, its important to save yourself some energy, otherwise you might die
of thirst!
...
'NORTH' + 'SOUTH' is going north and coming back right away.
...
"""
def get_optimized_path(path: list) -> list:
    path_len = len(path)

    while True:
        # Внутрішній цикл – це один прохід оптимізації маршруту
        i = 0
        while i < len(path) - 1:
            if ((path[i] == 'WEST' and path[i + 1] == 'EAST')
                    or (path[i] == 'EAST' and path[i + 1] == 'WEST')
                    or (path[i] == 'SOUTH' and path[i + 1] == 'NORTH')
                    or (path[i] == 'NORTH' and path[i + 1] == 'SOUTH')):
                # Порядок видалення дуже важливий!
                del path[i + 1]
                del path[i]
                continue
            i += 1

        # Якщо довжина маршруту вже не змінюється, то оптимізація завершена
...
for i, path in enumerate(paths):
    ...
    print(path)
    print()
    print(f'Оптимізований маршрут #{i}:')
    print(get_optimized_path(path))
    print()
